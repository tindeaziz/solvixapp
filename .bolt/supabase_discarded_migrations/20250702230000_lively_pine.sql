/*
  ...
  1. Problem
    - The trigger function that creates a profile when a new user signs up is blocked by RLS policies
    - This causes "Database error saving new user" during registration
    - The issue is that auth.uid() is NULL during the trigger execution
  
  2. Solution
    - Modify RLS policies to allow the postgres role (which executes the trigger) to bypass RLS
    - Add OR session_user = 'postgres' to the RLS policies
    - Keep all existing security measures intact
*/
