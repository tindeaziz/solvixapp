...
const CreateQuote: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 1. Créer ou récupérer le client si nécessaire
      let clientId = null;
      ...
      console.log('✅ CREATE_QUOTE - Devis sauvegardé avec succès');
      setSaveSuccess(true);
      setTimeout(() => setSaveSuccess(false), 3000);

      // Générer un nouveau numéro pour le prochain devis
      const nextQuoteNumber = await devisService.generateQuoteNumber();
      setQuoteData(prev => ({ ...prev, number: nextQuoteNumber }));
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          <div className="flex items-center space-x-3 sm:space-x-4">
            ...
            <div>
              ...
              <p className="text-gray-600 font-inter text-sm sm:text-base">Générez un devis professionnel en quelques clics</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-solvix border border-gray-200 p-4 sm:p-6">
            ...
            <div className="flex flex-wrap gap-3 mt-6 pt-4 border-t border-gray-200">
              ...
              <button
                onClick={handleSave}
                disabled={saving}
                className="inline-flex items-center px-4 py-2 bg-solvix-blue text-white rounded-lg text-sm font-medium hover:bg-solvix-blue-dark transition-colors duration-200 disabled:opacity-50 font-inter shadow-solvix"
              >
                {saving ? (
                  <>
